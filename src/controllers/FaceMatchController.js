    ...
    try{
        ...
        try {
            if(req.body['accuracy']==="high"){
                ...
                FaceDetails1 = await faceapi.detectSingleFace(img1).withFaceLandmarks().withFaceDescriptor()
                FaceDetails2 = await faceapi.detectSingleFace(img2).withFaceLandmarks().withFaceDescriptor()
            }
            else{
                ...
                FaceDetails1 = await faceapi.detectSingleFace(img1,new TinyFaceDetectorOptions()).withFaceLandmarks().withFaceDescriptor()
                FaceDetails2 = await faceapi.detectSingleFace(img2,new TinyFaceDetectorOptions()).withFaceLandmarks().withFaceDescriptor()
            }

            let distance = faceapi.euclideanDistance(FaceDetails1['descriptor'], FaceDetails2['descriptor']);
            let similarity = 1-distance;

            res.status(200).json({status: "success", message: "Face detected", distance: distance, similarity: similarity})
